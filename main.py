...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        if not self.check_ffmpeg():
            messagebox.showwarning(
                ...
                "Video/audio processing may fail.\n"
        ...
        # Preview Frame
        self.video_title = customtkinter.CTkLabel(
            self.preview_frame, text="Video title will appear here", wraplength=600, font=("Segoe UI", 16, "bold")
        )
        ...
        self.download_btn = customtkinter.CTkButton(
            self.action_frame, text="Download Best Video+Audio", command=lambda: self.start_download(video=True)
        )
        self.download_btn.grid(row=1, column=0, padx=(10, 5), pady=5, sticky="ew")

        self.audio_btn = customtkinter.CTkButton(
            self.action_frame, text="Download Audio (MP3)", command=lambda: self.start_download(video=False)
        )
    ...
    def start_fetch_info(self):
        ...
        if not ytlink:
            ...
            return
        if not self.validate_url(ytlink):
